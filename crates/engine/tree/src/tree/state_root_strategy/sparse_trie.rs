    ...
    S: SparseTrie + Default + Clone + 'static,
{
    ...
    fn step(
        ...
        match event {
            ...
            SparseTrieEvent::Storage(returned) => self.on_storage_job_message(returned)?,
        }
    ...
    fn receive_event(
        &self,
        blocking: bool,
    ) -> Result<Option<SparseTrieEvent<S>>, StateRootTaskError> {
            ...
            ($($default:tt)*) => {
                crossbeam_channel::select_biased! {
                    ...
                    recv(self.storage_done_rx) -> message => {
                        Ok(Some(SparseTrieEvent::Storage(
                            message.expect("we own the storage sender")
                        )))
                    },
