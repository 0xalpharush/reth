    ...
    DB: EvmStateProvider,
{
    ...
    fn get_block_hash(&mut self, number: &Word) -> Result<B256, Self::Error> {
        Ok(normalize_provider_block_hash(
...
#[cfg(feature = "std")]
fn normalize_provider_block_hash(hash: Option<B256>) -> B256 {
...
mod tests {
    ...
    fn missing_provider_block_hash_matches_revm_zero_hash_adapter() {
        assert_eq!(normalize_provider_block_hash(None), B256::ZERO);
        let hash = B256::with_last_byte(1);
        assert_eq!(normalize_provider_block_hash(Some(hash)), hash);
    }
}
