...
static FORCE_NATIVE: AtomicBool = AtomicBool::new(false);

/// Routes parallel operations to native Rayon for differential runs in a `dst` process.
pub fn force_native() {
    FORCE_NATIVE.store(true, AtomicOrdering::Release);
}
...
pub fn is_inline() -> bool {
    INLINE.get() && !FORCE_NATIVE.load(AtomicOrdering::Acquire)
}
